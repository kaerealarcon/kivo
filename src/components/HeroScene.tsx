...
const vertexShader = /* glsl */`
  ...
  void main() {
    ...
    // Layered sine-wave blob deformation
...
const fragmentShader = /* glsl */`
  ...
  void main() {
    ...
    // Hue cycles with time + normal direction
    float hue  = fract(n.x * 0.22 + n.y * 0.28 + n.z * 0.18 + uTime * 0.045);
    float hue2 = fract(hue + 0.38);

    vec3 base  = hsl2rgb(hue,  0.72, 0.52);
    vec3 edge  = hsl2rgb(hue2, 0.88, 0.68);

    vec3 color = mix(base, edge, fr);
    color += fr * fr * 0.45;               // specular white
    color = mix(color * 0.85, color, fr);  // darken center slightly

    gl_FragColor = vec4(color, 0.82 + fr * 0.16);
  }
`
...
export default function HeroScene() {
  ...
  useEffect(() => {
    ...
    // ─── Iridescent blob ───────────────────────────────────────
    const geo = new THREE.SphereGeometry(2.1, 128, 128)
    const uniforms = {
      uTime:   { value: 0 },
    ...
    // ─── Faint outer ring ──────────────────────────────────────
    const ringGeo = new THREE.TorusGeometry(3.0, 0.008, 8, 160)
    ...
    const pMat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.025, transparent: true, opacity: 0.4 })
